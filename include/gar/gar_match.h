...
inline bool MatchCallbackSaveResultWithCheckXY(
    const std::map<QueryVertexPtr, 
               DataGraphVertexPtr> &m, 
    const LiteralList &x,
    const LiteralList &y, 
    std::set<QueryVertexPtr> &pivot, 
      SuppContainer *supp,
    ResultContainer *r) {
...
int GenXYParticalMatch(GAR &gar, DataGraph &target,
                       ResultContainer &match_result) {
  using   PatternVertexConstPtr = typename GUNDAM::VertexHandle<typename GAR::PatternType>::type;
  ...
  auto user_call_back = std::bind(
      MatchCallbackSaveResultWithCheckXY<
          check_y_flag, PatternVertexConstPtr, 
                      DataGraphVertexConstPtr,
          ResultContainer, 
           LiteralSetType, 
        SuppContainerType>,
      std::placeholders::_1, std::ref(x_literal_set), 
                             std::ref(y_literal_set),
                             std::ref(pivot), &supp, &match_result);
  auto prune_call_back = std::bind(
      GARPruneCallBack<PatternVertexConstPtr, 
                     DataGraphVertexConstPtr,
                           SuppContainerType, 
                              LiteralSetType>,
      std::placeholders ::_1, std::ref(pivot), 
                              std::ref(y_literal_set), &supp);
...
template <bool check_y_flag, class GAR, class DataGraph, class ResultContainer>
int CalCompleteMatch(GAR &gar, DataGraph &target,
  ...
  for (auto &match_state : xy_result) {
    ...
    auto user_callback = std::bind(
        GUNDAM::_dp_iso::MatchCallbackSaveResult<
...
template <bool check_y_flag, class GAR, class DataGraph, class ResultContainer,
          class = typename std::enable_if<std::is_convertible<
              std::pair<typename GUNDAM::VertexHandle<typename GAR::PatternType>::type,
                        typename GUNDAM::VertexHandle<DataGraph>::type>,
              typename ResultContainer::value_type::value_type>::value>::type>
inline int GARMatch(GAR &gar, DataGraph &target,
                    ResultContainer &match_result) {
  ResultContainer xy_result;
