...
static std::string timer_now() {
  ...
  double time = timer() - timers.back().second ;

  const std::string& name = timers.back().first;

  return " - " + name + ": " + std::to_string(time) + " sec";
}

static double timer_now_time() {
  if (!timer_enabled) {
    return 0.0;
  }
  if (timers.empty()) {
  ...
  return (timer() - timers.back().second) ;
}
